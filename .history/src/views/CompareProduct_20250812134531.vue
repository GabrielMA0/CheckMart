...
const schema = yup.object({
  ...
  .string()
  .transform((value) => {
    if (!value) return '' // vazio continua vazio
  ...
  unit1: yup.number().when('unitCheckbox', {
    is: true,
    then: schema => schema.required('Preencha o campo obrigatório'),
    otherwise: schema => schema.notRequired(),
  }),

  volume1: yup.number().when('volumeCheckbox', {
    is: true,
    then: schema => schema.required('Preencha o campo obrigatório'),
    otherwise: schema => schema.notRequired(),
  }),

  weight1: yup.number().when('weightCheckbox', {
    is: true,
    then: schema => schema.required('Preencha o campo obrigatório'),
    otherwise: schema => schema.notRequired(),
  }),
  ...
  unit2: yup.number().when('unitCheckbox', {
    is: true,
    then: schema => schema.required('Preencha o campo obrigatório'),
    otherwise: schema => schema.notRequired(),
  }),

  volume2: yup.number().when('volumeCheckbox', {
    is: true,
    then: schema => schema.required('Preencha o campo obrigatório'),
    otherwise: schema => schema.notRequired(),
  }),

  weight2: yup.number().when('weightCheckbox', {
    is: true,
    then: schema => schema.required('Preencha o campo obrigatório'),
    otherwise: schema => schema.notRequired(),
  }),
...
const getCheapestProduct = (products) => {
  if (products[0].price < products[1].price) {
    return products[0];
  } else if (products[1].price < products[0].price) {
    return products[1];
  } else {
    return null; // preços iguais
  }
}

const compareProduct = handleSubmit((values) => {

  const priceParse1 = parseFloat(values.price1.slice(3).replace(',', '.'))
  const priceParse2 = parseFloat(values.price2.slice(3).replace(',', '.'))
    ...
    <Container>
        ...
        <form v-show="!teste" @submit.prevent="compareProduct">
            ...
            <div class="container-btns">
                ...
                <Button type="button" @click="resetForm">Limpar campos</Button>
                <router-link to="/">
